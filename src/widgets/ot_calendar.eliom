...
let get_rotated_days { i_days; i_start } =
  if List.length i_days != 7 then invalid_arg "intl.i_days"
  else rotate_list i_days (int_of_dow i_start)

let name_of_calendarlib_month { i_months } m =
  if List.length i_months != 12 then invalid_arg "intl.i_months"
...
let rec build_calendar ?prehilight
  ...
  and f_cell i j =
    ...
    if A.month fst_dow = A.month d then
      ...
      let classes =
        [
          ( if d < today then "ot-c-past"
          else if d = today then "ot-c-today"
          else "ot-c-future" );
        ]
      in
      let classes =
...
let%client update_classes cal zero d =
  ...
  let f i =
    ...
    let f j =
      ...
      in
      if d = d' then c ##. classList ## (add (Js.string "ot-c-current"))
...
let%client attach_events ?action ?(click_non_highlighted = false) ?update ~intl
  ...
  if mnth = A.month d then
    ...
    let set_onclick () =
      ...
      Lwt.async f
    in
    if List.exists (( = ) dom) highlight then (
      c ##. classList ## (add (Js.string "ot-c-highlight"));
      set_onclick () )
    else if click_non_highlighted then set_onclick ()
    else ()
